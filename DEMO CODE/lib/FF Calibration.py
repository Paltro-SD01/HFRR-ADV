"""
HFRR Friction Force Calibration Report Generator
=================================================
...
LabVIEW Python Node call signature:
    generate_friction_report(
        ...
        pass_or_fail,        # string, explicitly "PASS" or "FAIL"
...
Save this file e.g. as: C:\\HFRR_Calibration\\generate_friction_report.py
LabVIEW Python Node settings:
    Module Name : generate_friction_report
    Function    : generate_friction_report
"""
...
BORDER_BLUE_LINE = HexColor("#1A73E8")  # Explicit blue for curved panel border
...
# Curved Box Paragraph Styles
spring_constant_style = ParagraphStyle(
    "SpringConstantStyle", parent=styles["Normal"], fontSize=10, fontName="Helvetica-Bold",
    textColor=SLATE_PRIMARY, spaceAfter=3
)
spring_force_style = ParagraphStyle(
    "SpringForceStyle", parent=styles["Normal"], fontSize=8, fontName="Helvetica-Oblique",
    textColor=SLATE_SECONDARY
)
...
def generate_friction_report(
    ...
    """
    Generate a Friction Force Calibration PDF report matching requested templates.
    """

    # --- Structural Mapping & Safety Checks -----------------------------
    pass_fail = str(pass_or_fail).strip().upper()
    if pass_fail not in ["PASS", "FAIL"]:
        pass_fail = "FAIL"

    calibration_name = "Friction Force Calibration Report"
    ...
    filename = f"Friction_Force_Calibration_{instrument_serial}_{file_safe_time}.pdf"
    ...
    # --- Curved Box Mechanical Metadata Block -------------------------
    # Renders a stylized card wrapped in an explicit bold blue border line
    spring_box_data = [[
        [
            Paragraph("Spring constant : K = 1.00 N/mm", spring_constant_style),
            Paragraph("Spring force |F| = k * d", spring_force_style)
        ]
    ]]
    spring_box_table = Table(spring_box_data, colWidths=[170 * mm])
    spring_box_table.setStyle(TableStyle([
        ["BACKGROUND", (0, 0), (-1, -1), BG_LIGHT_PANEL],
        ["TOPPADDING", (0, 0), (-1, -1), 10],
        ["BOTTOMPADDING", (0, 0), (-1, -1), 10],
        ["LEFTPADDING", (0, 0), (-1, -1), 12],
        ["RIGHTPADDING", (0, 0), (-1, -1), 12],
        ["BOX", (0, 0), (-1, -1), 1.5, BORDER_BLUE_LINE], # Bold Blue border line
    ]))
    story.append(spring_box_table)
    ...
    # Selection logic mapping color metrics directly to requested strings
    if pass_fail == "PASS":
        status_card_bg = GREEN_CARD_BG
        status_card_text_color = GREEN_CARD_TXT
        status_comment = "Calibration completed successfully. The measured friction force is within the acceptable tolerance range of the factory calibration value."
        status_text_style = pass_status_style
    else:
        status_card_bg = RED_CARD_BG
        status_card_text_color = RED_CARD_TXT
        status_comment = "Calibration failed. The measured friction force differs from the factory calibration value . Please perform the calibration again."
        status_text_style = fail_status_style

    # Status Panel View Block
    ...
    # Text assessment commentary printout
...
if __name__ == "__main__":
    print("=== HFRR Friction Force Report Generator Test Interface ===")
    eval_input = input("Enter operational verification status (PASS or FAIL): ")
    
    print("\nProcessing friction mechanical metrics...")
    
    generated_path = generate_friction_report(
        instrument_serial="HFRR-FRIC-TRANS",
        operator_name="Mechanical System Tester",
        pass_or_fail=eval_input,
        output_dir="./test_output",
        logo_path="",
        logo_link_url=LOGO_LINK_URL,
        footer_link_url=FOOTER_LINK_URL
    )
    print(f"\nProcessing execution complete. PDF saved to:\n{generated_path}")