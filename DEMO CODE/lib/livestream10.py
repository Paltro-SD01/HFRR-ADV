...
# ================= CONFIGURATION =================
WINDOW_WIDTH, WINDOW_HEIGHT = 1400, 850
WINDOW_X, WINDOW_Y = 16, 203
            ...
            try:
                with conn:
                    while True:
                        ...
                        if cmd == CMD_CONNECT: 
                            try:
                                # Try to turn the payload into a number
                                cam_index = int(payload)
                                self.sig_connect_camera.emit(cam_index)
                            except ValueError:
                                # If it's a '/' or other text, don't crash! Default to camera 0.
                                print(f"⚠ Warning: Received invalid camera index '{payload}'. Defaulting to camera 0.")
                                self.sig_connect_camera.emit(0)
